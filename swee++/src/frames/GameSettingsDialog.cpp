	...
	: wxDialog(parent, -1, _("Game settings"))
{
	...
	this->mFieldWidth = new wxSpinCtrl(fieldSB, -1, wxEmptyString, wxPoint(10, 20), wxSize(130, 22), wxSP_ARROW_KEYS, FieldSizeMin, FieldSizeMax, fieldWidth);
	this->mFieldHeight = new wxSpinCtrl(fieldSB, -1, wxEmptyString, wxPoint(145, 20), wxSize(130, 22), wxSP_ARROW_KEYS, FieldSizeMin, FieldSizeMax, fieldHeight);
	
	this->mMines = new wxSpinCtrl(minesSB, -1, wxEmptyString, wxPoint(10, 20), wxSize(265, 22), wxSP_ARROW_KEYS, MinesMin, static_cast<int>(fieldWidth * fieldHeight * MinesMaxMultiplier), fieldMines);
	
	sizer->Add(panel, 1);
	sizer->Add(this->CreateButtonSizer(wxOK), 1, wxALIGN_RIGHT | wxDOWN | wxRIGHT, 10);
...
{
	this->mMines->SetMax(static_cast<int>(this->mFieldWidth->GetValue() * this->mFieldHeight->GetValue() * MinesMaxMultiplier));
}
